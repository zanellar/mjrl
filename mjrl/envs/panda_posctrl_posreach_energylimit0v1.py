...
class Environment(PandaPosCtrlPosReachEnv): 
  ...
  def step(self, action):  
    ...
    # Debug 
    if self.debug:
      print(f"energy_exiting={self.energy_out} ")
  ...
  def get_obs(self):  
    ...
    normalized_energy_out = self.energy_out/self.energy_margin 
    qtor = np.array(self.sim.get_joints_ft()[0:7])   

    # Concatenate observations
    obs = np.concatenate([
      nominal_obs,
      np.array([normalized_energy_out]),
      np.tanh(qtor)
    ]).astype(np.float32)  
