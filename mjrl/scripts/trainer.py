...
class Trainer():
    ...
    def run(self):   
        ...
        try: 
            weights_path = os.path.join(self.settings.weights_path, self.settings.env_id, self.settings.run_id) 
        except:
            weights_path = os.path.join(WEIGHTS_PATH, self.settings.env_id, self.settings.run_id)
        
        if not os.path.exists(weights_path):
            os.makedirs(weights_path)

        best_model_folder_path = os.path.join(weights_path, "best_model")
